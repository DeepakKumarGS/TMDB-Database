...
# Loading the data and summary {.tabset .tabset-pill}
...
#Wordcloud-Genre Representation;
...
production %>% group_by(company) %>% summarise(count=n()) %>% arrange(desc(count)) %>% with(wordcloud(words=company,freq=count,max.words = 25,color =rainbow(7),scale=c(2,0.2)))
```
...
```{r}
par(mfrow=c(1,2))
wordcloud(positive$text,positive$freq_tag, min.freq=7,rot.per=0.15,colors=rainbow(15),vfont=c("sans serif","plain"))
text(x=0.5, y=1.05, "Positive",col="black")
wordcloud(negative$text,negative$freq_tag, min.freq=7,colors=rainbow(15))
text(x=0.5, y=1.05, "Negative",col="black",rot.per=0.15,vfont=c("sans serif","plain"))
```
...
df_credit %>% ggplot(aes(factor(name,levels=name),n,fill=name))+geom_bar(stat="identity")+labs(x="Artist",y="Count",title="Top 10 artist with most movies")+theme_few()+theme(axis.text.x=element_text(angle=90),plot.title=element_text(hjust=0.5,color="red"),legend.position="none")
```
...
Compared to Tom,here we see that less budget movies are more popular than the movies made at higher budget.